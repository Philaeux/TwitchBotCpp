...
void TwitchChatClient::connect() {
    _socket.close();
    tcp::resolver resolver{_io};
    auto endpoints = resolver.resolve("irc.chat.twitch.tv", "6667");
    boost::system::error_code error;
    auto finalEndpoint = asio::connect(_socket, endpoints.begin(), endpoints.end(), error);
    ...
    asio::async_read_until(
        ...
        _in_buffer, 
        "\r\n", 
        boost::bind(&TwitchChatClient::on_socket_line, this, asio::placeholders::error, asio::placeholders::bytes_transferred)
    );
};

void TwitchChatClient::say(std::string message) {
    send_line("PRIVMSG #" + _cfg.irc_channel() + " :" + message);
};

void TwitchChatClient::login() {
...
void TwitchChatClient::on_socket_line(const boost::system::error_code& error, size_t s) {
...
void TwitchChatClient::on_line(std::string line) {
    std::smatch matches;
    if (line.size() > 5 && line.substr(0, 6) == "PING :"){
        ...
        if (reward_id == ""){
            on_message(user, is_mod, is_sub, message);
        } else {
            on_reward(user, reward_id, message);
        }
    }
};

void TwitchChatClient::on_message(std::string user, bool is_mod, bool is_sub, std::string message) {
    std::cout << user;
    if(is_mod)
        std::cout << "[M]";
    if(is_sub)
...
void TwitchChatClient::on_reward(std::string user, std::string reward_id, std::string message){
    std::cout << user << "(" << reward_id << "): " << message << std::endl;
};

void TwitchChatClient::send_line(std::string data) {
    data += "\r\n";
...
void TwitchChatClient::socket_send() {
    if(_out_buffer.empty()){
        return;
    }
    
    _socket.async_send(
        asio::buffer(_out_buffer.front().data(), _out_buffer.front().size()), 
        boost::bind(&TwitchChatClient::on_socket_sent, this, asio::placeholders::error, asio::placeholders::bytes_transferred)
    );
};

void TwitchChatClient::on_socket_sent(const boost::system::error_code& error, size_t s) {
    if(error) {
